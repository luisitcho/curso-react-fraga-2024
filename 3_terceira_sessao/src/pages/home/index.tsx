    ...
    return (
        <main className={`container ${style.container}`}>
            <blockquote className="blockquote text-center">
                <p className="text-white">Aqui na minha corretora, todo mundo fica rico RAPÁ</p>
                <footer className="blockquote-footer text-white"><cite title="Source Title">Luisitcho</cite></footer>
            </blockquote>
            ...
            <form className={style.form} onSubmit={handleSubmit}>
                <input type="text" placeholder="Digite o nome da moeda... EX bitcoin" className='form-control' value={input} onChange={(e) => setInput(e.target.value)} />
                <button type="submit">
                        ...
                        {coins && coins.map((coin) => (
                            <tr className={style.tr} key={coin.id}>
                                ...
                                <td className={style.tdLabel} data-label="Volume">
                                    <span>{coin.volumeUsd24HrFormated}</span>
                                </td>

                                <td className={Number(coin.changePercent24Hr) > 0 ? style.tdProfit : style.tdLoss} data-label="Mudança 24h">
            ...
            <button className="btn btn-dark" onClick={handleGetMore}>Carregar mais</button>
        </main>
    )
}